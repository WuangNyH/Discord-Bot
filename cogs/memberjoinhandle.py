...
class MemberJoinHandle(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.Cog.listener()
    async def on_ready(self):
        print("MemberJoinHandle is ready!")
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        await welcome_channel.send(f"Hello there, {member.mention}! Thank you for joining our server!")
        await welcome_channel.send(file=img_file)

async def setup(bot):
    await bot.add_cog(MemberJoinHandle(bot))
